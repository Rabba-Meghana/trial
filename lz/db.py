...
class ParticipantRow(Base):
    ...
    __table_args__ = (UniqueConstraint("organization_id", "external_id", name="uq_org_participant"),)

    id: Mapped[str] = mapped_column(String(64), primary_key=True, default=lambda: f"pt_{uuid4().hex}")
...
class RiskLimitRow(Base):
    ...
    __table_args__ = (UniqueConstraint("organization_id", "participant_id", "currency", name="uq_risk_limit"),)

    id: Mapped[str] = mapped_column(String(64), primary_key=True, default=lambda: f"rl_{uuid4().hex}")
    organization_id: Mapped[str] = mapped_column(String(64), index=True)
...
class TransferRow(Base):
    ...
    __table_args__ = (UniqueConstraint("organization_id", "idempotency_key", name="uq_transfer_idem"),)

    id: Mapped[str] = mapped_column(String(64), primary_key=True, default=lambda: f"tr_{uuid4().hex}")
    organization_id: Mapped[str] = mapped_column(String(64), index=True)
...
class ReconciliationRow(Base):
    ...
    __table_args__ = (UniqueConstraint("organization_id", "external_reference", name="uq_recon_ref"),)

    id: Mapped[str] = mapped_column(String(64), primary_key=True, default=lambda: f"rec_{uuid4().hex}")
    organization_id: Mapped[str] = mapped_column(String(64), index=True)
...
class WebhookEndpointRow(Base):
    ...
    id: Mapped[str] = mapped_column(String(64), primary_key=True, default=lambda: f"wh_{uuid4().hex}")
    organization_id: Mapped[str] = mapped_column(String(64), index=True)
